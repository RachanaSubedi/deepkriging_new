...
if __name__ == "__main__":
    ...
    print("     → place in data/raw/goes_c13/extracted_pixels/ alongside")
    print("       the other pixel CSVs (c13_features.py auto-discovers")
    print("       every CSV in that folder via load_all_pixels())")
    print("  3. NOTE: config.py has no per-station raw-file mapping to edit")
    print("     (no cfg.RAW dict exists). To actually route S3 to this new")
    print("     pixel instead of the shared S2/S3 one, S3's row in")
    print("     data/processed/pv_pixel_map.csv needs its pixel_id changed")
    print("     to 's3_standalone' — verify this against pixel_mapping.py's")
    print("     actual output format before editing.")
    print(f"{'─'*60}")