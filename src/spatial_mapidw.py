"""
src/idw_spatial_map.py

Spatial GHI visualisation across 178 PV locations, using the IDW
baseline's output (outputs/idw/ghi_pvs_idw.parquet). Mirrors
spatial_map.py exactly so DeepKriging and IDW figures are
directly comparable side by side — same snapshot logic, same
colour scale convention, same file layout, only the input file
and output filenames (idw_ prefix) differ.

Produces (outputs/figures/):
  fig_idw_{TARGET_DATE}_timeseries.png   all PV predictions vs station measurements
  fig_idw_{TARGET_DATE}_spatial.png      4 spatial snapshots across the day
  fig_idw_spatial_4panel.png             best clear / partly-cloudy / overcast auto-selected

Run:
    python src/idw_spatial_map.py
"""
...
from configs.config import FIG_DIR, STATIONS, OUTPUT_DIR

# ── Which date to plot — keep in sync with spatial_map.py's TARGET_DATE
#    if you want a direct side-by-side comparison ──
TARGET_DATE = "2024-5-22"

# ── PATHS ─────────────────────────────────────────────────────
PRED_CSV    = OUTPUT_DIR / "idw" / "ghi_pvs_idw.parquet"
...
for col in pv_names:
    ax.plot(daytime.index, daytime[col], color='darkorange',
            lw=0.9, alpha=0.3, zorder=1)

pv_med = daytime.median(axis=1)
ax.plot(daytime.index, pv_med, color='darkorange', lw=2,
        label='PV median prediction (IDW)', zorder=3)

for col in pv_names[::10]:
    ax.plot(daytime.index, daytime[col], color='moccasin',
...
ax.set_title(f'{pd.Timestamp(TARGET_DATE).strftime("%B %d, %Y")} - '
             f'IDW-Predicted GHI at 178 PV Locations vs Station Measurements\n',
...
out = OUTPUT_DIR / "idw" / f"fig_idw_{date_tag}_timeseries.png"
plt.tight_layout()
...
print(f"\n  Snapshot times on {TARGET_DATE} (IDW):")
...
fig.suptitle(f'{pd.Timestamp(TARGET_DATE).strftime("%B %d, %Y")} — IDW-Predicted GHI Spatial Distribution\n'
...
out = OUTPUT_DIR / "idw" / f"fig_idw_{date_tag}_spatial.png"
plt.tight_layout()
plt.savefig(out, dpi=160, bbox_inches='tight')
plt.close()
print(f"  ✓ {out.name}")
...
# Uses the SAME selection logic as spatial_map.py so panel dates
# should land close to (often identical to) DeepKriging's, making
# the two 4-panel figures directly comparable.
# ══════════════════════════════════════════════════════════════
...
fig.suptitle('IDW-Predicted GHI — Spatial Distribution Under Different Sky Conditions\n'
...
out = OUTPUT_DIR / "idw" / "fig_idw_spatial_4panel.png"
plt.tight_layout()
plt.savefig(out, dpi=150, bbox_inches='tight')
plt.close()
print(f"  ✓ {out.name}")

print(f"\n✓ All IDW figures saved to {FIG_DIR}")