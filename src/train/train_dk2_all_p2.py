...
def configure_experiment(
    ...
    if spatial_kind == "wendland":
        ...
        EXPERIMENT_NAME = f"DK2_planar_eigen_TPS_K016_allP2_seed42"
...
def main() -> None:
    ...
    # Exploratory full-P2 policy:
    # treat every P2 row, including reconstructed rows, as an ordinary station target.
    ground_truth_mask = np.ones(timestamps_ns.shape, dtype=bool)

    p2_fold_index = STATION_NAMES.index("P2")
    p2_rows = int(np.sum(fold_ids == p2_fold_index))

    provenance_summary = {
        "p2_all_available_rows": p2_rows,
        "p2_rows_in_training_matrix": p2_rows,
        "p2_reconstructed_rows_excluded": 0,
    }
    ...
    print(
        "P2 rows retained     : "
        f"{provenance_summary['p2_all_available_rows']:,} / "
    ...
    run_config = {
        ...
        "p2_training_policy": "all_available",
        "p2_testing_policy": "all_available",
        "p2_all_available_rows": provenance_summary["p2_all_available_rows"],
