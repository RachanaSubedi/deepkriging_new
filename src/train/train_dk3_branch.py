"""Train DK-3: parameter-matched branch architecture with TPS K=16.

Place this file at:
    src/train/train_dk3_branch.py
...
Run the first controlled comparison (seed 42):
    python src/train/train_dk3_branch.py --seed 42 --p2-provenance \
        "data/raw/stations/station_p2_full_year_GHI_zeroshot.csv"

Controlled comparison:
    comparator = monolithic TPS-K16 + coarse-30d temporal basis
    DK-3       = two-branch TPS-K16 + coarse-30d temporal basis

Everything else is inherited from train_dk0_matched.py: direct-CSI target,
Huber loss, optimizer, enhancement oversampling, shared
...
from src.model_branch import BranchDeepKriging, count_parameters  # noqa: E402
...
EXPERIMENT_LABEL = "Unconfigured DK-3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train direct-CSI DK-3 with a parameter-matched branch network "
    ...
    parser.add_argument(
        ...
        default="eigen_tps",
        help="Fixed to eigen_tps for DK-3 (argument retained for auditability).",
    )
    parser.add_argument(
        "--n-spatial-basis",
        type=int,
        default=16,
        help="Fixed to 16 for DK-3 (argument retained for auditability).",
    )
    parser.add_argument(
        "--temporal-levels",
        nargs="+",
        ...
        help=(
            "Fixed to coarse_30d for DK-3 (argument retained for auditability)."
        ),
    )
    parser.add_argument(
        "--p2-provenance",
...
def configure_experiment(
    ...
    if spatial_kind != "eigen_tps" or spatial_k != 16:
        raise ValueError("DK-3 fixes the spatial representation to eigen_tps K=16")
    if canonical != ("coarse_30d",):
        raise ValueError("DK-3 fixes the temporal representation to coarse_30d")
    EXPERIMENT_NAME = f"DK3_branch_TPS_K016_seed{seed}_groundtruth"
    EXPERIMENT_LABEL = f"DK-3 Branch Architecture, TPS K=16, seed={seed}"
    EXPERIMENT_DIR = OUTPUT_DIR / "experiments" / EXPERIMENT_NAME
    MODEL_DIR = EXPERIMENT_DIR / "models"
    VAL_DIR = EXPERIMENT_DIR / "validation"
    FIG_DIR = EXPERIMENT_DIR / "figures"
...
def train_branch_fold(
...
) -> tuple[BranchDeepKriging, list[dict[str, float]]]:
    """Same optimizer/loss/early stopping as the monolithic comparator."""
    model = BranchDeepKriging(
    ...
    if X_train.shape[1] != model.input_dim:
        raise ValueError(
            f"Branch model expects {model.input_dim} inputs; got {X_train.shape[1]}"
        )
...
def main() -> None:
    ...
    temporary_model = BranchDeepKriging(
        n_spatial_basis=n_spatial_basis,
        n_temporal_basis=n_temporal_basis,
        n_covariates=N_COVARIATES_EXPECTED,
        branch_width=BRANCH_WIDTH,
    ...
    for fold_k, test_station in enumerate(STATION_NAMES):
        ...
        start = time.time()
        model, history = train_branch_fold(
    ...
    (VAL_DIR / "loso_summary.txt").write_text(
        "DK-3 Branch Architecture Ground-Truth LOSO Results\n"
        + "=" * 60
        + "\n"
        + summary_text
        + "\n\n"
    ...
    run_config = {
        ...
        "single_intended_change": "monolithic_to_parameter_matched_two_branch_architecture",
        "target": "direct_csi",
        "input_order": [
        ...
        "architecture": "two_branch_basis_atmospheric_parameter_matched_mlp",
        "basis_branch_width": BRANCH_WIDTH,
        "atmospheric_branch_width": BRANCH_WIDTH,
        "branch_embedding": BRANCH_EMBEDDING,
        "fusion_width": FUSION_WIDTH,
        ...
        "branch_model_sha256": sha256_file(
            REPO_ROOT / "src" / "model_branch.py"
        ),
    }
    ...
    print("\nDK-2 monolithic and earlier outputs were not modified.")
