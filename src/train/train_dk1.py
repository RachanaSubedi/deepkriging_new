"""Train DK-1: direct CSI with spatial + temporal basis functions.

Place this file at:
    src/train/train_dk1.py

Prerequisites:
    python src/train/shared_blocked_day_split.py --validation-fraction 0.20
    python src/data_prep/temporal_basis.py

Run DK-1A (30-day temporal level only):
    python src/train/train_dk1.py --temporal-levels coarse_30d \
        --p2-provenance \
        "data/raw/stations/station_p2_full_year_GHI_zeroshot.csv"

Run DK-1B (30-day + 7-day temporal levels):
    python src/train/train_dk1.py \
        --temporal-levels coarse_30d medium_7d \
        --p2-provenance \
        "data/raw/stations/station_p2_full_year_GHI_zeroshot.csv"

Controlled change from DK0_matched_groundtruth:
    DK-0 input = [spatial Wendland basis, 15 covariates]
    DK-1 input = [spatial Wendland basis, temporal Gaussian basis,
                  15 covariates]
...
def configure_experiment(selected_levels: list[str]) -> tuple[str, ...]:
...
def assemble_dk1_features(
    ...
    phi = selected[:, :n_spatial_basis]
    covariates = selected[:, n_spatial_basis:]
...
def main() -> None:
    args = parse_args()
    selected_levels = configure_experiment(args.temporal_levels)
    ...
    if X_dk0.shape[1] != n_spatial_basis + N_COVARIATES_EXPECTED:
        raise ValueError(
            f"Expected {n_spatial_basis} spatial bases + "
    ...
    for fold_k, test_station in enumerate(STATION_NAMES):
        ...
        X_train = assemble_dk1_features(
            X_dk0, train_indices_expanded, n_spatial_basis,
            temporal_row_for_training_row, psi_unique,
        )
        X_val = assemble_dk1_features(
            X_dk0, val_indices, n_spatial_basis,
            temporal_row_for_training_row, psi_unique,
        )
        X_test = assemble_dk1_features(
            X_dk0, test_indices, n_spatial_basis,
            temporal_row_for_training_row, psi_unique,
        )
        ...
        # Phi and Psi remain in [0, 1]. Only the final 15 covariates are
        # standardized using statistics fitted on this fold's training rows.
    ...
    run_config = {
        ...
        "single_intended_change": "append_multiresolution_temporal_basis",
        "target": "direct_csi",
        "input_order": [
            "spatial_wendland_basis",
