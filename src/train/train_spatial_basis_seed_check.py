"""Seed robustness check for the two selected spatial-basis finalists.

Place this file at:
    src/train/train_spatial_basis_seed_check.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Independent training seed. Use 123 and 2026; seed 42 already exists.",
    )
    parser.add_argument(
        "--spatial-basis",
...
def configure_experiment(
    ...
    if seed == SEED:
        raise ValueError(
            f"Seed {SEED} is already represented by the reference runs; "
            "use seed 123 or 2026."
        )
    if spatial_kind == "wendland":
        ...
        if canonical != ("coarse_30d",):
            raise ValueError("Seed check fixes temporal levels to coarse_30d")
        EXPERIMENT_NAME = f"DK2_seedcheck_wendland_K411_seed{seed}_groundtruth"
        EXPERIMENT_LABEL = f"Seed Check: Wendland K=411, seed={seed}"
    else:
        if canonical != ("coarse_30d",):
            raise ValueError("DK-2 spatial comparison fixes temporal levels to coarse_30d")
        if spatial_k != 16:
            raise ValueError("Seed robustness is restricted to selected eigen-TPS K=16")
        EXPERIMENT_NAME = f"DK2_seedcheck_eigen_TPS_K016_seed{seed}_groundtruth"
        EXPERIMENT_LABEL = f"Seed Check: Planar Eigen-TPS K=16, seed={seed}"
    EXPERIMENT_DIR = OUTPUT_DIR / "experiments" / EXPERIMENT_NAME
    MODEL_DIR = EXPERIMENT_DIR / "models"
    VAL_DIR = EXPERIMENT_DIR / "validation"
    FIG_DIR = EXPERIMENT_DIR / "figures"
...
def main() -> None:
    ...
    run_config = {
        ...
        "seed": args.seed,
        "experiment_role": "spatial_basis_seed_robustness_check",
    ...
    print("\nReference outputs were not modified.")
