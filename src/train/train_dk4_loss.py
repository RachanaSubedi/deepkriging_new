"""Train DK-4: controlled Huber-versus-MSE loss comparison.

Place this file at:
    src/train/train_dk4_loss.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["mse", "huber"],
        default="mse",
        help="Training loss. Use mse for DK-4; huber is available only as an audit rerun.",
    )
    parser.add_argument(
...
def configure_experiment(
    ...
    EXPERIMENT_NAME = f"DK4_loss_{loss_name.upper()}_TPS_K016_seed{seed}_groundtruth"
    EXPERIMENT_LABEL = f"DK-4 {loss_name.upper()} Loss, TPS K=16, seed={seed}"
    EXPERIMENT_DIR = OUTPUT_DIR / "experiments" / EXPERIMENT_NAME
    MODEL_DIR = EXPERIMENT_DIR / "models"
    VAL_DIR = EXPERIMENT_DIR / "validation"
    FIG_DIR = EXPERIMENT_DIR / "figures"
...
def main() -> None:
    ...
    (VAL_DIR / "loso_summary.txt").write_text(
        f"DK-4 {args.loss.upper()} Loss Ground-Truth LOSO Results\n"
        + "=" * 60
        + "\n"
        + summary_text
        + "\n\n"
    ...
    run_config = {
        ...
        "single_intended_change": "HuberLoss_to_MSELoss" if args.loss == "mse" else "audit_Huber_rerun",
        "target": "direct_csi",
        "input_order": [
        ...
        "dropout": DROPOUT,
        "loss": "MSELoss" if args.loss == "mse" else "HuberLoss",
        "huber_delta": HUBER_DELTA if args.loss == "huber" else None,
        "optimizer": "Adam",
    ...
    print("\nMonolithic Huber comparator and earlier outputs were not modified.")
